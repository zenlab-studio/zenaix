// ══════════════════════════════════════════════════════
// Z-CHAT — Messaggi P2P + edit/delete + preview + reply
// ══════════════════════════════════════════════════════
const {
...
exports.handler = async (event) => {
  ...
  if (!account.zchatHandle) {
    return jsonResponse(400, { error: 'Devi prima creare il tuo @handle.' });
  }
  ...
  if (event.httpMethod === 'GET') {
    ...
    if (!conv) return jsonResponse(200, { messages: [], convId });

    if (!conv.participants.includes(myHandle)) {
      return jsonResponse(403, { error: 'Non sei partecipante.' });
    }
    ...
    return jsonResponse(200, { messages, convId, updatedAt: conv.updatedAt });
  }
  ...
  if (event.httpMethod === 'PATCH') {
    ...
    if (!convId || !messageId) return jsonResponse(400, { error: 'convId e msgId richiesti.' });

    const conv = await getConversation(convId);
    if (!conv || !conv.participants.includes(myHandle)) {
      return jsonResponse(403, { error: 'Conversazione non accessibile.' });
    }

    const idx = (conv.messages || []).findIndex(m => m.id === messageId);
    if (idx < 0) return jsonResponse(404, { error: 'Messaggio non trovato.' });
    const msg = conv.messages[idx];
    if (msg.from !== myHandle) return jsonResponse(403, { error: 'Puoi modificare solo i tuoi messaggi.' });
    ...
    return jsonResponse(200, { message: msg });
  }
  ...
  if (event.httpMethod === 'POST') {
    ...
    if (!toHandle) return jsonResponse(400, { error: 'Destinatario mancante.' });
    if (toHandle === myHandle) return jsonResponse(400, { error: 'Non puoi scriverti.' });

    const recipientEntry = await getHandleIndex(toHandle);
    if (!recipientEntry) return jsonResponse(404, { error: 'Destinatario non trovato.' });

    if (type === 'text' && !text.trim() && !body.encrypted) {
      return jsonResponse(400, { error: 'Messaggio vuoto.' });
    }
    if (['image', 'video', 'file', 'gif', 'design'].includes(type) && !media) {
      return jsonResponse(400, { error: 'Allegato mancante.' });
    }
    if (media?.dataUrl && media.dataUrl.length > MAX_MEDIA_SIZE * 1.4) {
      return jsonResponse(400, { error: 'File troppo grande.' });
    }

    const recipientAcc = await getAccountByUsername(recipientEntry.username);
    if (!recipientAcc) return jsonResponse(404, { error: 'Account destinatario non trovato.' });

    const access = canReceiveFrom(recipientAcc, myHandle);
    if (!access.ok && access.reason === 'blocked') {
      return jsonResponse(403, { error: 'Sei stato bloccato.' });
    }
    ...
    if (!access.ok && access.reason === 'request') {
      ...
      return jsonResponse(202, { pending: true, requestMessage: message });
    }
    ...
    return jsonResponse(201, { message, convId });
  }
